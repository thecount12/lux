...
dictInitNative(int argCount, Value* args)
{
	...
	Dict *d = dict_new();
	if (d == NULL) return NIL_VAL;
	tableSet(&inst->fields, copyString("_ptr", 4), NUMBER_VAL((double)(uintptr_t)d));
	return args[0];
}
...
dictClearNative(int argCount, Value* args)
{
	...
	Dict *d = getDictFromInstance(args[0]);
	if (d == NULL) return NIL_VAL;
	dict_iter(d, free_value_cb, NULL);
	dict_clear(d);
	return NIL_VAL;
}
