...
ninepFill(ObjInstance* fs, NinePFile* files, int* nfiles)
{
	...
	for (i = 0; i < arr->count && n < NINEP_MAXFILES; i++) {
		...
		Value nameVal, readVal, writeVal;
		ObjInstance* ent;
		const char* name;
...
static int
ninepListenPath(ObjInstance* fs, const char* path)
{
	int srv, client;
...
ninepListenNative(int argCount, Value* args)
{
	if (argCount != 2 || !IS_INSTANCE(args[0]) || !IS_STRING(args[1]))
		return BOOL_VAL(false);
	if (ninepListenPath(AS_INSTANCE(args[0]), AS_CSTRING(args[1])) < 0)
		return BOOL_VAL(false);
	return BOOL_VAL(true);
}
...
ninepPostNative(int argCount, Value* args)
{
	...
	if (ninepListenPath(AS_INSTANCE(args[0]), path) < 0)
		return BOOL_VAL(false);
	return BOOL_VAL(true);
}
