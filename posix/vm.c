...
static void httpStripHostPort(char* host) {
	char* p;
	char* q;
...
static int httpHeaderNameIs(char* line, const char* name) {
	int i;
	for (i = 0; name[i] != '\0'; i++) {
		char a = line[i];
		char b = name[i];
...
static void httpCopyHeaderValue(char* line, char* lineEnd, int nameLen, char* dst, int dstSz) {
	char* v;
	int n;
...
static int resGetFd(ObjInstance* res) {
	Value fdVal;
...
static void resEmit(ObjInstance* res, int statusCode, const char* contentType,
...
static int serverRouteOpArg(int argCount, Value* args, int opIndex) {
	if (argCount < opIndex)
		return 0;
	if (argCount == opIndex)
		return 1;
...
static void serverRouteSetOp(ObjInstance* entry, int argCount, Value* args, int opIndex) {
...
static Value serverGetHostNative(int argCount, Value* args) {
...
static Value serverPostHostNative(int argCount, Value* args) {
	if (!serverRouteOpArg(argCount, args, 4) || !IS_INSTANCE(args[0]) ||
	    !IS_STRING(args[1]) || !IS_STRING(args[2]) || !IS_CLOSURE(args[3]))
		return NIL_VAL;
	ObjInstance* server = AS_INSTANCE(args[0]);
...
/* Lookup static root for Host from _vhosts; NULL if none. */
static char* serverLookupVhostRoot(ObjInstance* server, char* host) {
	Value vhostsVal;
	ObjArray* vhosts;
	int i;
	...
	for (i = 0; i < vhosts->count; i++) {
		...
		if (strcasecmp(AS_CSTRING(hostVal), host) == 0)
			return AS_CSTRING(rootVal);
	}
	return NULL;
}

static ObjClosure* serverFindHandler(ObjArray* routes, const char* method,
		const char* path, const char* host) {
	ObjClosure* handler = NULL;
	int pass;

	if (routes == NULL || method == NULL || path == NULL)
		return NULL;
	if (host == NULL)
		host = "";
	for (pass = 0; pass < 2 && handler == NULL; pass++) {
		...
		for (i = 0; i < routes->count; i++) {
			...
			if (pass == 0) {
				if (!hasHost) continue;
				if (strcasecmp(AS_CSTRING(hostVal), host) != 0) continue;
			} else {
				if (hasHost) continue;
			}
...
static int serverTryStatic(ObjInstance* server, ObjInstance* res,
	...
	}
	{
		...
		}
		{
			...
			resEmit(res, 200, mime, content, (int)nread);
			free(content);
		}
	}
	fclose(f);
...
static void serverDispatch(ObjInstance* server, const char* method, const char* path,
		const char* host, const char* body, const char* authorization, ObjInstance* resObj) {
	ObjInstance* reqObj;
...
static void httpCopyUpper(char* dst, int dstSz, const char* src) {
	int i = 0;
	if (dstSz <= 0) return;
	while (src != NULL && src[i] != '\0' && i < dstSz - 1) {
		char c = src[i];
...
static void httpPathWithoutQuery(char* dst, int dstSz, const char* src) {
	int i = 0;
	if (dstSz <= 0) return;
	if (src == NULL || src[0] == '\0') {
		if (dstSz > 1) {
...
/* Server.handle(method, path, [body], [host], [authorization]) — one request, no socket.
   Returns an instance with statusCode, body, contentType. */
static Value serverHandleNative(int argCount, Value* args) {
	ObjInstance* server;
	ObjInstance* resObj;
	ObjInstance* out;
	const char* methodSrc;
	const char* pathSrc;
	const char* bodySrc = "";
	const char* hostSrc = "";
	const char* authSrc = "";
	...
	httpCopyUpper(methodBuf, (int)sizeof(methodBuf), methodSrc);
	httpPathWithoutQuery(pathBuf, (int)sizeof(pathBuf), pathSrc);
	snprintf(hostBuf, sizeof(hostBuf), "%s", hostSrc);
...
static Value serverStartNative(int argCount, Value* args) {
	...
	Value staticVal;
	const char* defaultStaticDir = NULL;
	if (tableGet(&server->fields, copyString("_static_dir", 11), &staticVal) && IS_STRING(staticVal))
