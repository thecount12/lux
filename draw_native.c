...
static int
parseColor(char *s, int *r, int *g, int *b)
{
	int i, v[6];

	if(s == nil || s[0] != '#' || strlen(s) != 7)
		return 0;
	for(i = 0; i < 6; i++){
		v[i] = hexnibble((uchar)s[i+1]);
		if(v[i] < 0)
			return 0;
	}
	*r = (v[0]<<4) | v[1];
...
drawAvailableNative(int argCount, Value *args)
{
	USED(argCount);
	USED(args);
	return BOOL_VAL(luxdraw_available());
}
...
drawEventNative(int argCount, Value *args)
{
	...
	tableSet(&inst->fields, copyString("kind", 4),
	         OBJ_VAL(copyString(kind, strlen(kind))));
